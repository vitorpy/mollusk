...
impl<'ix_data> TransactionContext<'ix_data> {
    ...
    /// Override the next top-level instruction index before the next [`Self::push`].
    ///
    /// Used by harnesses (for example Mollusk's `process_instruction_chain`) that run each
    /// instruction in an isolated [`TransactionContext`] but need the instructions sysvar to
    /// reflect the instruction's position within a logical chain.
    pub fn set_next_top_level_instruction_index(&mut self, index: usize) {
        self.next_top_level_instruction_index = index;
    }
